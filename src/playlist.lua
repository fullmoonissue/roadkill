...
local function compileItem(workItem, workItems)
    -- If a folder is present ...
    if workItem['folder'] ~= nil then
        -- ... list the valid files ...
        local workFiles = {}
        for _, workFile in ipairs(vlcWraps.readdir(workItem['folder'])) do
        ...
        for _, workFile in ipairs(workFiles) do
            ...
            for _, methodByOption in ipairs(vlcOptions.options) do
...
local function addItem(name, properties, playlistItems)
    if properties ~= nil then
        ...
        if properties['folder'] ~= nil then
            ...
            if path ~= nil then
                ...
                for _, optionMethod in ipairs(vlcOptions.options) do
                    if properties[optionMethod['option']] ~= nil then
                        table.insert(
                            item['options'],
                            vlcOptions.optionMethod(optionMethod['option'])(properties[optionMethod['option']])
                        )
                    end
                end
...
local function addItems(name, items, playlistItems)
    if items ~= nil then
...
local function compile(config, playlistItems)
